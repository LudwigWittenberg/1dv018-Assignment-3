...
def run_performance_test(sizes, num_tests=100):
    """Run performance tests for different array sizes with log2-based depths"""
    results = {
        'sizes': [],
        'dynamicsort_times': [],
        'quicksort_times': [],
...
def analyze_log2_depth():
    ...
    print("Testing DynamicSort with depth = log2(array_size)")
    print()
    ...
    # Depth analysis
    print(f"\n=== DEPTH ANALYSIS ===")
    print("Depth progression based on log2(array_size):")
    for i, size in enumerate(results['sizes']):
        depth = results['depths'][i]
        print(f"Array size {size:3}: depth = {depth} (log2({size}) = {math.log2(size):.2f})")
